...
from ev_mdt.analysis.sensitivity import (
    ...
    DEPARTURE_PROFILES, HORIZON_HOURS, build_params, _gaussian_pbp,
)
...
def make_result(label: str, params, t: int = T) -> dict:
    K = params.K
    return dict(
        label=label,
        params=params,
...
def horizon_results() -> list[dict]:
    return [make_result(f"{T_h} h", build_params(BASELINE_MODEL), T_h * 60)
...
def _add_heatmap(fig, r, pname, row, col, show_scale):
    ...
    t = r["T"]
    # Scale time_bin so all panels have ~1440 columns regardless of horizon length.
    time_bin = max(TIME_BIN_MIN, t // (24 * 60))
    probs = np.array([pbp_fn(i) for i in range(t)])
    ...
    z, t_c, b_c = _bin_heatmap(rates, e_grid, t, time_bin, BATTERY_BIN_KWH,
...
def fig_heatmaps_grid(results, pname, rows, cols) -> go.Figure:
    ...
    for i, r in enumerate(results):
        ...
        fig.update_xaxes(title_text="Hour (h)" if row == rows else "", range=[0, r["T"] // 60],
...
def main() -> None:
    ...
    hm = fig_heatmaps_grid(h_results, "Battery Level Urgency", rows=3, cols=1)
    (OUT / "horizon_battery_level_urgency.png").write_bytes(figure_to_png(hm))
    print(f"[horizon] saved BLU heatmaps")
